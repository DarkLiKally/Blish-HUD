...
public class StreamMessageService : IMessageService, IDisposable {
    ...
    private readonly ConcurrentDictionary<ulong, ManualResetEventSlim> waitingMessages   = new();
    private readonly ConcurrentDictionary<ulong, Message>              receivedMessages  = new();
    private readonly ConcurrentDictionary<Type, Action<Message>>       registedCallbacks = new();
    ...
    public void Register<T>(Action<T> callback) where T : Message { registedCallbacks.AddOrUpdate(typeof(T), _ => x => callback((T)x), (_, _) => x => callback((T)x)); }
