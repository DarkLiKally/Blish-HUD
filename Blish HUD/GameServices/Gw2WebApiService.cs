...
public class Gw2WebApiService : GameService {
    ...
    private readonly ConcurrentDictionary<string, string>            _characterRepository = new();
    private readonly ConcurrentDictionary<string, ManagedConnection> _cachedConnections   = new();
    ...
    private void RegisterApiInSettings() {
        // Manage API Keys
        Overlay.SettingsTab.RegisterSettingMenu(new MenuItem(Strings.GameServices.Gw2ApiService.ManageApiKeysSection, Content.GetTexture("155048")),
                                                            (_) => new RegisterApiKeyView(),
                                                            int.MaxValue - 11);
    }
    ...
    private async Task UpdateCharacterList(SettingEntry<string> definedKey) {
        try {
            ...
            foreach (string characterId in characters) {
                _characterRepository.AddOrUpdate(characterId, definedKey.Value, (_, _) => definedKey.Value);
            }
