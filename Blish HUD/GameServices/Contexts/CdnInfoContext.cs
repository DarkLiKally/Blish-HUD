    ...
    private async Task<string> GetCdnInfoFromCdnUrl(string cdnUrl) {
        try {
            ...
            if (ex.Call.Response != null) {
                Logger.Warn(ex, "Failed to get CDN information from {cdnUrl}.  HTTP response status was ({httpStatusCode}) {statusReason}.", cdnUrl, ex.Call.Response.StatusCode, ex.Call.Response.ResponseMessage.ReasonPhrase);
            } else {
