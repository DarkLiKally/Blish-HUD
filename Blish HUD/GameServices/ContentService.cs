...
public class ContentService : GameService {
    ...
    private static readonly ConcurrentDictionary<string, BitmapFont> _loadedBitmapFonts = new();
    private static readonly ConcurrentDictionary<string, Texture2D>  _loadedTextures    = new();
    ...
    public void PlaySoundEffectByName(string soundName) {
        ...
        if (GameIntegration.Audio.AudioDevice == null) {
        ...
        try {
            ...
            if (_audioDataReader.FileExists(filePath)) {
                SoundEffect.FromStream(_audioDataReader.GetFileStream(filePath)).Play(GameIntegration.Audio.Volume, 0, 0);
            }
    ...
    public MonoGame.Extended.TextureAtlases.TextureAtlas GetTextureAtlas(string textureAtlasName) {
        return Content.ContentManager.Load<MonoGame.Extended.TextureAtlases.TextureAtlas>(textureAtlasName);
    }
    ...
    public Texture2D GetTexture(string textureName, Texture2D defaultTexture) {
        ...
        if (cachedTexture == null) {
            try {
                cachedTexture = Content.ContentManager.Load<Texture2D>(textureName);
    ...
    private static readonly Regex _regexRenderServiceSignatureFileIdPair = new(@"(.{40})\/(\d+)(?>\..*)?$", RegexOptions.Singleline | RegexOptions.Compiled);
